...
local function SyncKillsFromLockouts()
    ...
    for i = 1, n do
        ...
        -- Defensive: API can return numEnc as string (or empty) in some
        -- 12.0.5 cases. Coerce to number before comparison.
        numEnc = tonumber(numEnc) or 0
        if ok and locked and numEnc > 0 then
            for j = 1, numEnc do
...
local function OnTargetChanged(unit)
    ...
    -- 12.0.5: In raid/M+/arena/PvP, enemy unit data is tainted at the deepest
    -- level — UnitName, UnitGUID return paths, even GetInstanceInfo() for the
    -- name field can carry secret strings into our table lookups and explode
    -- somewhere downstream. We skip target-based detection ENTIRELY in those
    -- contexts and rely on ENCOUNTER_START (clean encounterID arg) to show
    -- the cheat sheet when the pull actually happens. Open world / friendly
    -- target hover still works.
    local _, instType = IsInInstance()
    if instType == "raid" or instType == "party"
       or instType == "arena" or instType == "pvp" then
        return
    end
    ...
    -- Method 1: Try UnitName (works for friendly/neutral NPCs, outdoor bosses).
    -- 12.0.5: UnitName returns a "secret string" in raids/M+/arenas. We skip
    -- this method entirely in those contexts — even reading the value into a
    -- local can taint downstream code paths, and string ops trigger errors
    -- caught by pcall but loud as "Target error:" prints. Method 2 (GUID)
    -- works fine on tainted units, so loss of Method 1 is harmless in
    -- instances. Open world / friendly targets still go through here.
    local _, instType = IsInInstance()
    local stringsTainted = (instType == "raid" or instType == "party"
        or instType == "arena" or instType == "pvp")
    if not stringsTainted then
        local rawName = UnitName(unit)
        if rawName then
            local scrubbed = VoidLib and VoidLib.Secrets
                and VoidLib.Secrets.SafeKey(rawName) or rawName
            local ok, result = pcall(FindBoss, scrubbed)
    ...
    if not ok2 then
        pcall(function()
            local errStr = VoidLib and VoidLib.Secrets
                and VoidLib.Secrets.SafeKey(err2) or "(secret-string error)"
            print(C_RED .. "[VoidCheatSheet] ShowTooltip error: " .. errStr .. "|r")
        end)
    end
end
...
ef:SetScript("OnEvent", function(_, event, arg1, arg2, arg3, arg4, arg5)
    ...
    if event == "PLAYER_TARGET_CHANGED" then
        ...
        if not ok then
            -- 12.0.5: err itself can be a secret string. Wrap the print so
            -- a tainted err doesn't re-explode and spam every target swap.
            local printOk = pcall(function()
                local errStr = VoidLib and VoidLib.Secrets
                    and VoidLib.Secrets.SafeKey(err) or "(secret-string error)"
                print(C_RED .. "[VoidCheatSheet] Target error: " .. errStr .. "|r")
            end)
            if not printOk then
                -- Silent if even the safe-print failed
            end
        end
        return
    end
